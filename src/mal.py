...
short_field_type = lambda fields: [
    (short_name(field), field_type) for field, field_type in fields
]
INFO_TO_TYPE = {
    0: EXPORT_FIELDS,
...
def parse_json(path: str, from_string: bool = False) -> dict:
    ...
    for key in (
        "my_rewatching" if list_type == ListType.ANIME else "my_rereading",
        "my_discuss",
        "update_on_import",
    ):
...
def get_last_export(
    list_type: ListType = ListType.ANIME,
    user_id: int = USER_ID,
    deep: bool = False,
) -> dict:
    ...
    for export in reversed(EXPORTS):
        ...
        # get the data from the filename
            ...
            if export["user"] == user_id and export["type"] == list_type:
                return parse_export(export["path"])
    raise ValueError(
        f"{enum_str(list_type)} list " f"not found for user {user_id}."
    )
...
def dict_view(data: dict) -> dict:
    ...
    return {entry[title_field]: entry for entry in data[list_type]}
...
def cdata_wrap(key: str, value: str) -> str:
    ...
    return (
        f"<![CDATA[{value.replace(chr(10), chr(10))}]]>"
        if key in FIELDS_WRAP
        else value
    )
...
def data_to_json(data: dict) -> str:
    ...
    return to_json(
        {
            "info": serialize_data(
                data["info"], INFO_TO_TYPE[list_type], TO_JSON
            ),
            type_name: [
...
def data_to_xml(data: dict) -> str:
    ...
    entry_to_str = partial(
        serialize_data, field_to_type=DATA_TO_TYPE[list_type], wrap=True
    )
    eol, tab = "\n", "\t"
...
{indent}\t<myinfo>
{eol.join(
    f"{indent}{2*tab}<user_{key}>{value}</user_{key}>"
...
{indent}{"".join(
    ...
    f"{eol.join(f'{5*tab}<{k}>{v}</{k}>' for k, v in entry.items())}"
